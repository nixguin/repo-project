...
export default function AdminPanel() {
  ...
  const logAction = (msg: string) => setActionLog((prev) => [msg, ...prev].slice(0, 10));
  ...
  const handleBan = (id: string) => {
    ...
    logAction(`[${new Date().toLocaleTimeString()}] ${u.status === "banned" ? "Unbanned" : "Banned"} user: ${u.username}`);
  };
  ...
  const handlePromote = (id: string) => {
    ...
    const u = users.find((u) => u.id === id)!;
    logAction(`[${new Date().toLocaleTimeString()}] Promoted ${u.username} to admin`);
  };
  ...
  const handleDelete = (id: string) => {
    ...
    if (u.role === "organizer") {
      logAction(`[${new Date().toLocaleTimeString()}] BLOCKED: Cannot delete organizer ${u.username} with active tournaments`);
      return;
    }
    setUsers((prev) => prev.filter((u) => u.id !== id));
    logAction(`[${new Date().toLocaleTimeString()}] Deleted user: ${u.username}`);
  };
  ...
  return (
    <Layout>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Action Log (REQ-08)</h2>
        {actionLog.length === 0 ? (
