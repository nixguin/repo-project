...
type Filter = "all" | "upcoming" | "live" | "completed" | "competitive" | "casual";
...
export default function EventsPage() {
  ...
  const filtered = eventList.filter((e) => {
    ...
    if (filter === "competitive" || filter === "casual") return e.type === filter;
  ...
  return (
    <Layout>
          ...
          <div className={`rounded-2xl p-5 mb-6 border ${
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              {hero.status !== "completed" && (
                <button
                  onClick={() => registeredIds.has(hero.id) || hero.registeredPlayers < hero.maxPlayers
                ...
                >
                  {registeredIds.has(hero.id) ? "âœ“ Registered" : hero.registeredPlayers >= hero.maxPlayers ? "Full" : "Register Now"}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {(["all", "upcoming", "live", "completed", "competitive", "casual"] as Filter[]).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
              ...
              <div className="p-5">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium shrink-0 ${typeStyle[ev.type]}`}>
                        {typeLabel[ev.type]}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {ev.status !== "completed" && (
                      ...
                      >
                        {isRegistered ? "âœ“ Registered (Withdraw?)" : isFull ? "Full" : "Register"}
                      </button>
                    )}
                    {ev.status === "completed" && (
                      <span className="text-xs text-gray-400 px-3 py-1.5">Event ended</span>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <div className="flex justify-between text-xs text-gray-500 mb-1">
                    ...
                    <span>{ev.registeredPlayers} / {ev.maxPlayers} players</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-1.5">
                    <div
                      className={`h-1.5 rounded-full transition-all ${
                        fillPct >= 100 ? "bg-red-500" : fillPct >= 75 ? "bg-yellow-500" : "bg-fgcu-emerald"
                      }`}
                      style={{ width: `${fillPct}%` }}
                    />
                  </div>
                  {ev.status === "upcoming" && (
                    <p className="text-xs text-gray-400 mt-1">Registration closes: {ev.registrationDeadline}</p>
                  )}
                </div>
              </div>
